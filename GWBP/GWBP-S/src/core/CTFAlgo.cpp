...
{
	int nxb      = nx_pad + 2 - (nx_pad&1);
	int loop_num = nxb * ny_pad;
    int i;
...
static void calc_psd_2D(float *image,float *psd,int Nx,int Ny)
{
	...
	float *bufc = (float *)aligned_alloc(4096, sizeof(float) * ( Nx + 2 - Nx % 2) * Ny);
    ...
    int fft_x_offset = int(ceil(float(Nx+1)/2))-1;
	int fft_y_offset = int(ceil(float(Ny+1)/2))-1;
	...
	for(int j = 0; j < Ny; j++)
	{
		int j_psd  = fft_y[j] + fft_y_offset;
		int j_loop = j * loop_num;

		if(j_psd>=0 && j_psd<Ny)
		
		for(int i = 0; i < loop_num; i +=2)
		{
			register int i_psd = fft_x[i>>1] + fft_x_offset;
			if(i_psd>=0 && i_psd<Nx)
            {
                float bufc_tmp1, bufc_tmp2;
                bufc_tmp1 = bufc[i + j_loop];
                bufc_tmp2 = bufc[i + 1 + j_loop];

                bufc_tmp1 *= bufc_tmp1;
                bufc_tmp2 *= bufc_tmp2;
                bufc_tmp1 += bufc_tmp2;

                psd[i_psd+j_psd*(Nx)] = bufc_tmp1;

                psd[(Nx-i_psd-1)+(Ny-j_psd-1)*(Nx)] = bufc_tmp1;
            }
		}
	}
...
static void calc_psd_2D_padding(float *image,float *psd,int Nx,int Ny)
{
	...
	float * bufc = (float *) aligned_alloc(4096, sizeof(float) * (Nx_pad+2) * Ny_pad);
    ...
    int fft_x_offset = int(ceil(float(Nx_pad+1)/2))-1;
	int fft_y_offset = int(ceil(float(Ny_pad+1)/2))-1;
	...
	float * psd_padding = (float *)aligned_alloc(4096, sizeof(float) * Nx_pad * Ny_pad);
