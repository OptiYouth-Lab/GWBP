...
extern "C" void filter_weighting_1d_many_gpu_inplace(
...

    {
        ...
        pad_rows_linear_kernel<<<grid, block>>>(
...

    {
        ...
        build_weight_lut_kernel<<<bpg, tpb>>>(
        ...
        apply_weight_kernel<<<grid, block>>>(
...

    {
        ...
        unpack_rows_norm_kernel<<<grid, block>>>(
...
__global__ void ctf_phaseflip_expand_kernel(
    ...
    float alpha;
    if (fabsf(x_norm) < 1e-10f) {
        alpha = (y_norm > 0) ? (M_PI / 2.0f) : (y_norm < 0) ? (-M_PI / 2.0f) : 0.0f;
    } else {
        alpha = atan2f(y_real, x_real);
    }
    ...
    float df = 0.5f * (defocus1 + defocus2 - 2.0f * z_off * pix +
                       (defocus1 - defocus2) * __cosf(2.0f * (alpha - astig)));
...
struct CtfBatchCache {
    ...
    void destroy() {
        if (plan_fwd) { cufftDestroy(plan_fwd); plan_fwd = 0; }
        if (plan_inv_many) { cufftDestroy(plan_inv_many); plan_inv_many = 0; }
    ...
    void ensure(int nx, int ny, int b) {
        if (nx == Nx && ny == Ny && b == batch && plan_fwd && plan_inv_many && d_freq_base && d_freq_batch && d_z_offsets) return;
        destroy();
        Nx = nx; Ny = ny; batch = b;
        ...
        CUDA_CHECK(cudaMalloc(&d_freq_batch, freq_size * (size_t)batch * sizeof(cufftComplex)));
        CUDA_CHECK(cudaMalloc(&d_z_offsets,  (size_t)batch * sizeof(float)));
        ...
        int odist = Nx * Ny;
        CUFFT_CHECK(cufftPlanMany(&plan_inv_many,
                                 2, n,
...
extern "C" void ctf_correction_3d_gpu_device(
    ...
    if (z_offsets_host) {
        CUDA_CHECK(cudaMemcpy(g_ctf_cache.d_z_offsets, z_offsets_host,
                              (size_t)num_slices * sizeof(float),
                              cudaMemcpyHostToDevice));
    } else {
        CUDA_CHECK(cudaMemset(g_ctf_cache.d_z_offsets, 0, (size_t)num_slices * sizeof(float)));
    }
    ...
    normalize_kernel<<<bpg, tpb>>>(d_corrected, total, scale);
    CUDA_CHECK_LAST_ERROR();
}
