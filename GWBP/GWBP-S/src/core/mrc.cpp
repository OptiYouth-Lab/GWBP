...
int MRC::createMRC(float *data, int nx, int ny, int nz)
{
    ...
    for (i = 0; i < size; i++)
    {
        if (min_1[omp_get_thread_num()] > data[i])
        {
            min_1[omp_get_thread_num()] = data[i];
        }

        if (max_1[omp_get_thread_num()] < data[i])
        {
            max_1[omp_get_thread_num()] = data[i];
        }

        mean_1[omp_get_thread_num()] += data[i];
    }
...
int MRC::read2DIm_32bit(float *buf, int n)
{
    ...
    switch (mode)
    {
        case 0:
		{
            buf8 = (char *)aligned_alloc(64,size);
            //buf8 = new char[size];
            r = read2DIm((void *)buf8, n);
        ...
        case 1:
        {
            buf16 = (short *)aligned_alloc(64,size);
            r     = read2DIm((void *)buf16, n);
        ...
        case 5:
        {
            bufu8 = (unsigned char *)aligned_alloc(64,size);
            r     = read2DIm((void *)bufu8, n);
        ...
        case 6:
		{
            bufu16 = (unsigned short *)aligned_alloc(64,size);
            r = read2DIm((void *)bufu16, n);
